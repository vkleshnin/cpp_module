...
void Form::checkExecutable(Bureaucrat &_Bureaucrat) {
	if (this->getSigned()) {
		if (_Bureaucrat.getGrade() > this->gradeExec) {
			std::cout << _Bureaucrat.getName() << " couldn’t execute " << this->getName() << " because: ";
			throw Form::GradeTooLowException();
		}
	} else {
		std::cout << "The form " << this->getName() << " cannot be executed, because: ";
